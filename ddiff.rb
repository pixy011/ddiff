...
begin
  OptionParser.new do |options|
    ...
    options.on('-a', '--action ACTION', 'Possible actions: snapshot*, compare') do |action|
    ...
    options.on('-i', '--snapshot-in PATH', 'Snapshot to compare to') do |path|
    ...
    options.on('-o', '--snapshot-out PATH', 'Where to save the snapshot') do |path|
      DDiff::Config.instance.snapshot_out = path
    end

    options.on('-d', '--diff-out PATH', 'Where to save the diff report') do |path|
    ...
    options.on('-s', '--ignore', 'Comma separated list of absolute file paths to ignore (works for files only)') do |string|
